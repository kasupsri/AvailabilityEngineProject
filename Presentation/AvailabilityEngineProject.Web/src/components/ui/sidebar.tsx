...
function Sidebar({
  ...
  const gapWidth = state === "collapsed" && collapsible === "offcanvas" 
    ? 0 
    : parseFloat(sidebarWidth.replace('rem', '')) * 16;

  const sidebarLeft = state === "collapsed" && collapsible === "offcanvas" && side === "left"
    ? -parseFloat(sidebarWidth.replace('rem', '')) * 16
    : 0;
  const sidebarRight = state === "collapsed" && collapsible === "offcanvas" && side === "right"
    ? -parseFloat(sidebarWidth.replace('rem', '')) * 16
    : undefined;
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: `${gapWidth}px` } as React.CSSProperties}
      />
      <div
        className={cn(
        ...
        style={{ 
          width: sidebarWidth,
          left: side === "left" ? sidebarLeft : undefined,
          right: side === "right" ? sidebarRight : undefined,
        } as React.CSSProperties}
        {...props}
      >
        <div className="bg-sidebar flex h-full w-full flex-col border shadow-sm">
          {children}
        </div>
      </div>
    </div>
